...
inherit cros-riscv cros-arm64 cros-i686 cros-workon flag-o-matic multiprocessing
...
src_configure() {
  ...
  if use_riscv; then
    einfo "Configuring 64-bit riscv build"
    mkdir workv
    pushd workv > /dev/null
    use_riscv && push_riscv64_env
    ECONF_SOURCE=${S} multijob_child_init econf
    use_riscv && pop_riscv64_env
    popd > /dev/null
  fi
...
src_compile() {
  ...
  if use_riscv; then
    einfo "Building 64-bit riscv library"
    push_riscv64_env
    emake -C workv src/client/linux/libbreakpad_client.a
    pop_riscv64_env
  fi
}
...
src_install() {
  ...
  if use_riscv; then
    push_riscv64_env
    dolib.a workv/src/client/linux/libbreakpad_client.a
    pop_riscv64_env
  fi
}
